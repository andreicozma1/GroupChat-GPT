...
const createChatStartPrompt = (messages: TextMessage[]) => {
  ...
  res +=
    "When specifically asked to create an image, the assistant will ask it's friend DALL-E to help create an image.";
  res += "### Human:\nHello, who are you?\n\n";
  res +=
...
const createClassificationPrompt = (message: TextMessage[]) => {
  ...
  res += "###\n";
  res +=
    "Chat: I can ask my friend DALL-E to help create an image for you. Could you generate a drawing of a dog?" +
    " It's wearing a space suit and floating in space.\n";
  res += "Task: generate_image\n";
  res +=
    "Prompt: A drawing of a dog wearing a space suit and floating in space.\n\n";

  // last 10 messages from the user
  const lastMessages = message.slice(-10);
