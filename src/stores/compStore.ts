  ...
  return (
    getAssistantsList()
      .map((ai) => {
        ...
        res += ":\n";
        if (ai.personality)
          res += `- Personality Traits: ${ai.personality.join(", ")}.\n`;
        if (ai.strengths) res += `- Strengths: ${ai.strengths.join(", ")}.\n`;
        if (ai.weaknesses)
          res += `- Weaknesses: ${ai.weaknesses.join(", ")}.\n`;
        return res;
      })
      .join("\n") + "\n"
  );
...
const getPromptCoordinator = (actor: ActorConfig, messages: TextMessage[]) => {
  let res = "### COORDINATOR ###\n";
  res +=
    "Classify which assistants would be best at responding to the next message.";
  res += "Only respond with the exact names of the assistants\n";
  res +=
    "If the user's request is specifically directed at multiple assistants, separate the names with a comma\n";
  ...
  actions: {
    ...
    async genCompletion(actor: ActorConfig): Promise<GenerationResult> {
      ...
      try {
        ...
        if (error.response)
          errorMsg = `${error.response.status} ${JSON.stringify(
            error.response.data
          )}`;
        return {
          result: null,
    ...
    pushMessage(message: TextMessage): TextMessage {
      if (message.id) {
        ...
        const existingIdx = this.getThread.messages.findIndex(
          (m) => m.id === message.id
        );
        if (existingIdx !== -1) {
